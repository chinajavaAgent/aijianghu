            ...
            <div v-if="currentProject.cases?.length" 
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div v-for="(item, index) in currentProject.cases" :key="index"
                  class="bg-gradient-to-br from-amber-50 to-orange-50 rounded-lg p-4 hover:shadow-md transition-all duration-300 border border-amber-100">
                  <img v-if="item.imageUrl" 
                    :src="item.imageUrl" 
                    :alt="item.description" 
                    class="w-full h-48 object-cover rounded-lg mb-3 shadow-sm">
                  <div class="flex items-start">
                    <svg class="w-5 h-5 text-amber-500 mr-2 mt-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
